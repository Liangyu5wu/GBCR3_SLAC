...
def generate_summary(result_dir, dbg_mode=0):
    ...
    for line in lines:
        ...
        if chan_event[chan] == 0:
            ...
            start_dt = datetime.strptime(ch_date_time_trimmed, "%Y-%m-%d %H:%M:%S.%f")
            start_time[chan]   = start_dt
        ...
        minute = int(ch_date_time_trimmed[14:16])
        second = int(ch_date_time_trimmed[17:19])
        end_dt = datetime.strptime(ch_date_time_trimmed, "%Y-%m-%d %H:%M:%S.%f")
        end_time[chan]   = end_dt
        end_year[chan]   = year
